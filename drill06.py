...
def reset_world():
    ...
    points = [(100, 900), (1200, 800), (500, 1000)]
...
def update_world():
    ...
    if target_exists:
        if t <= 1.0:
            cx = (1 - t) * sx + t * hx  # cs는 시작 x와 끝 x를 1-t : t 비율로 섞은 위치
            cy = (1 - t) * sy + t * hy
            t += 0.001
        else: #목표 지점에 도달하면
            cx, cy = hx, hy  # 캐릭터 위치를 기존 목적지 위치와 정확히 일치시킴.
...
while running:
    ...
    update_world()  # 월드 안의 객체들에 상호작용을 예상하고 그 결과를 update한다.
